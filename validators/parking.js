...
module.exports = {
    checkGetParking() {
        return [
            check("name").optional()
                ...
                .isLength({max: 50}).withMessage("Parking name is maxlength 50"),
            check("id",).optional()
    ...
    checkAddParking() {
        return [
            check("name")
                .not().isEmpty().withMessage("Please input parking name")
                .isString().withMessage('Name must be a String')
                .isLength({max: 50}).withMessage("Parking name is maxlength 50")
                .custom(async (value) => {
            ...
            check("slot").not().isEmpty().withMessage("Please input slot")
                .isInt().withMessage("Slot  must be a Integer"),
            check("subParkings").isArray().withMessage("Sub parking format is invalid"),
            check("subParkings.*.name").not().isEmpty().withMessage("Please input sub parking name")
                .isString().withMessage('Sub parking name must be a String')
                .isLength({max: 50}).withMessage("Sub parking name is maxlength 50")
    ...
    checkUpdateParking() {
        return [
            ...
            check("subParkings").optional().isArray().withMessage("Sub parking format is invalid"),
            check("subParkings.*.name").optional()
                .isString().withMessage('Name must be a String')
                .isLength({max: 50}).withMessage("Sub parking name is maxlength 50")
                .custom(async (value, {req}) => {
                    if (req.body.subParkings.filter(o => o.name === value).length > 1)
            ...
            check("subParkings.*.isActive").optional()
                .isBoolean().withMessage("Sub parking is not correct")
